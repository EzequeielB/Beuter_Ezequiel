...
USE ventas;
/*Cambio el nombre de la columna para evitar el error "1052. column in field list is ambiguous" en futuros puntos al seleccionar "id"*/
ALTER TABLE clientes RENAME COLUMN id TO id_c;
ALTER TABLE vendedores RENAME COLUMN id TO id_v;
...
SELECT DISTINCT id_cliente, id_c FROM pedidos
JOIN clientes
ON id_cliente = id_c 
;
/* JOIN me asegura que si el valor es nulo en id_cliente no se agregue a la lista */
...
SELECT nombre, apellido, id_c FROM clientes  WHERE cuitcuil IS NOT NULL ORDER BY nombre, apellido ASC;
...
SELECT DISTINCT nombre_v, apellido_v FROM vendedores WHERE nombre_v LIKE '%el' OR nombre_v LIKE '%O' ORDER BY nombre_v ASC;
...
SELECT DISTINCT id_cliente, id_c, nombre, apellido FROM pedidos
JOIN clientes
ON id_cliente = id_c ;
...
SELECT nombre, apellido, nombre_v, apellido_v FROM pedidos 
JOIN clientes
ON id_cliente = id_c
JOIN vendedores
ON id_vendedor = id_v
WHERE nombre = 'Maria' AND apellido = 'Santana';
...
JOIN vendedores
ON id_vendedor = id_v
WHERE nombre_v = 'Daniel' AND apellido_V = 'Sáez';
...
ON id_vendedor = id_v
ORDER BY nombre_v, apellido_v ASC;
...
SELECT nombre AS 'Nombre_Cliente', apellido, NULL AS 'Apellido_Cliente', NULL AS 'Apellido_Vendedor' FROM pedidos 
RIGHT JOIN clientes
ON id_cliente = id_c 
WHERE id_cliente IS NULL
...
SELECT NULL, NULL, nombre_v, apellido_v FROM pedidos 
RIGHT JOIN vendedores
ON id_vendedor = id_v
WHERE id_vendedor IS NULL;
...
SELECT COUNT(nombre_v) AS CantidadDeVendedores FROM vendedores;
...
SELECT * FROM pedidos WHERE id_cliente = (SELECT id_C FROM clientes WHERE id_c=2);

/*38.Devuelve el número de pedidos en los que ha participado el vendedor Daniel Santana. (Sin utilizar INNER JOIN)*/

SELECT COUNT(id_cliente) AS NúmeroDeDPedidos FROM pedidos WHERE id_cliente = (SELECT id_c FROM clientes WHERE id_c=10);

/*39.Devuelve los datos del cliente que realizó el pedido más caro en el año 2020. (Sin utilizar INNER JOIN)*/

SELECT * FROM clientes where id_c = (SELECT id_cliente FROM pedidos WHERE fecha BETWEEN '2020-01-01' AND '2020-12-31' HAVING MAX(cantidad));

/*40.Devuelve la fecha y la cantidad del pedido de menor valor realizado por el cliente Pepe Ruiz Santana.*/

SELECT cantidad,fecha  FROM pedidos WHERE id_cliente = (SELECT id_c FROM clientes WHERE id_c=7) HAVING MIN(cantidad);
...
RIGHT JOIN clientes
ON id_cliente = id_c
WHERE id_c IN ((select id_c s WHERE id_cliente IS NULL))
;
...
RIGHT JOIN vendedores
ON id_vendedor = id_v
WHERE id_v IN ((select id_v WHERE id_vendedor IS NULL))
;
